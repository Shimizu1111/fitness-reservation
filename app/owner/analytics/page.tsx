...
import { TrendingUp, Users, Calendar, Clock, ArrowUp, ArrowDown, ChevronDown } from "lucide-react";
...
export default function AnalyticsPage() {
  ...
  // 時間帯ごとの予約数推移データの作成
  const timeSlotChartData = analytics.trends.timeSlots[0].dates.map((date, index) => {
    const data: any = { date };
  ...
  // レッスン種類別推移データの作成
  const lessonTypeChartData = analytics.lessonTypeStats[0].dates.map((date, index) => {
    const data: any = { date };
      ...
      <main className="container mx-auto p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                ...
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={analytics.trends.customers}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Line type="monotone" dataKey="total" name="総会員数" stroke="#0ea5e9" />
                      <Line type="monotone" dataKey="active" name="アクティブ会員" stroke="#22c55e" />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </Card>
          ...
          <div className="relative">
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <h2 className="text-sm font-medium text-gray-500">月間レッスン数</h2>
                    <ChevronDown className={`h-4 w-4 transition-transform ${expandedCard === 'lessons' ? 'rotate-180' : ''}`} />
                  </div>
                  <div className="flex items-center gap-2">
                    <p className="text-2xl font-bold text-gray-800">{analytics.currentStats.totalLessons}回</p>
                    <div className={`flex items-center text-sm ${lessonGrowth.isPositive ? 'text-green-500' : 'text-red-500'}`}>
                      {lessonGrowth.isPositive ? <ArrowUp className="h-4 w-4" /> : <ArrowDown className="h-4 w-4" />}
                      {lessonGrowth.value}%
                    </div>
                  </div>
                </div>
                ...
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={analytics.trends.lessons}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Line type="monotone" dataKey="count" name="レッスン数" stroke="#0ea5e9" />
                      <Line type="monotone" dataKey="attendance" name="出席率" stroke="#eab308" />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </Card>
          ...
          <div className="relative">
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <h2 className="text-sm font-medium text-gray-500">平均出席率</h2>
                    <ChevronDown className={`h-4 w-4 transition-transform ${expandedCard === 'attendance' ? 'rotate-180' : ''}`} />
                  </div>
                  <p className="text-2xl font-bold text-gray-800">{analytics.currentStats.averageAttendance}%</p>
                  <p className="text-sm text-gray-500">
                    前月比: {analytics.trends.lessons[1].attendance - analytics.trends.lessons[0].attendance}%
                  </p>
                </div>
              </div>
            </Card>
                ...
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={analytics.trends.lessons}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Line type="monotone" dataKey="attendance" name="出席率" stroke="#0ea5e9" />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </Card>
          ...
          <div className="relative">
            <Card 
              className="p-6 cursor-pointer"
              onClick={() => setExpandedCard(expandedCard === 'timeSlots' ? null : 'timeSlots')}
            >
              <div className="flex items-center gap-4">
                <Clock className="h-8 w-8 text-sky-500" />
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <h2 className="text-sm font-medium text-gray-500">人気の時間帯推移</h2>
                    <ChevronDown className={`h-4 w-4 transition-transform ${expandedCard === 'timeSlots' ? 'rotate-180' : ''}`} />
                  </div>
                  <div className="space-y-2 mt-2">
                    {analytics.trends.timeSlots.map((slot, index) => (
                      <div key={index} className="flex items-center gap-2">
                        <span className="text-sm text-gray-600 w-12">{slot.time}</span>
                        <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                          <div
                            className="h-full bg-sky-500 rounded-full"
                            style={{
                              width: `${(slot.counts[slot.counts.length - 1] / 50) * 100}%`
                            }}
                          />
                        </div>
                      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* レッスン種類別推移 */}
          <Card className="p-6">
            <h2 className="text-xl font-semibold text-gray-800 mb-6">レッスン種類別推移</h2>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
